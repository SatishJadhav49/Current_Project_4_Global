...
{
    public class MfgApprovalMailService : BaseMailService
    {
        public MfgApprovalMailService(
        ...
        /// <summary>
        /// Sends mail notification when action plan is submitted by employee to manager
        /// </summary>
        public async Task<bool> SendMailAsync(decimal concernId)
        {
            try
            {
                ...
                var concernDetails = await GetConcernDetailsForSubmissionMailAsync(concernId);
                ...
                var recipients = await GetMailRecipientsForSubmissionAsync(concernDetails);

                // Prepare mail body
                string mailBody = await PrepareMailBodyForSubmissionAsync(concernDetails);

                // Send email with multiple recipients
                await SendEmailWithRecipientsAsync(recipients, 
                    $"MCAR Action Plan Review Required - {concernDetails.Source_Name} - {concernDetails.Shop_Name} - {concernDetails.Stage_Name} - {concernDetails.Severity_Name}", 
                    mailBody);
            ...
            {
                throw new Exception($"Error sending submission mail for concern {concernId}: {ex.Message}", ex);
            }
        }

        private async Task<string> PrepareMailBodyForSubmissionAsync(SubmissionMailDataDto concernDetails)
        {
            try
            {
                // Read mail template
                string mailTemplate = await ReadMailTemplateAsync("mfg-approval-template.html");

                // Replace placeholders with actual data
                string mailBody = ReplacePlaceholdersInTemplate(mailTemplate, GetReplacementDictionaryForSubmission(concernDetails));

                return mailBody;
            }
            catch (Exception ex)
            {
                throw new Exception($"Error preparing mail body for submission concern {concernDetails.Concern_ID}: {ex.Message}", ex);
            }
        }

        private Dictionary<string, string> GetReplacementDictionaryForSubmission(SubmissionMailDataDto data)
        {
            string plantName = GetPlantName(data.Plant_Code);

            // Calculate review due date using the constant
            var reviewDueDate = data.Submission_Date.AddDays(Constants.SUBMISSION_REVIEW_Days);
            ...
            return new Dictionary<string, string>
            {
                ...
                { "[REVIEW_DUE_DATE]", reviewDueDate.ToString("dd-MMM-yyyy hh:mm tt") },
        ...
        private async Task<SubmissionMailDataDto?> GetConcernDetailsForSubmissionMailAsync(decimal concernId)
        {
            try
            {
                const string query = @"
                    SELECT 
                        ...
                        ISNULL(pl.Inserted_Date, GETDATE()) as Submission_Date,
                        ...
                        -- Manager Info (Reporting Manager of Assigned User)
                    ...
                    LEFT JOIN MM_Process_Logs pl ON cd.Concern_ID = pl.Concern_ID AND pl.Process_ID = @SubmissionProcessId
                ...
                if (await reader.ReadAsync())
                {
                    ...
                    return new SubmissionMailDataDto
                    {
            ...
            {
                throw new Exception($"Error retrieving concern details for submission mail: {ex.Message}", ex);
            }

            return null;
        }

        private async Task<MailRecipientsDto> GetMailRecipientsForSubmissionAsync(SubmissionMailDataDto concernData)
        {
            ...
            try
            {
                ...
                // Get TO recipients: Only the reporting manager of assigned user
                GetToRecipientsForSubmissionAsync(connection, concernData, recipients);

                // Get CC recipients: Same as assigned concern + Auditor
                await GetCcRecipientsForSubmissionAsync(connection, concernData, recipients);

                return recipients;
            }
            catch (Exception ex)
            {
                throw new Exception($"Error getting mail recipients for submission concern {concernData.Concern_ID}: {ex.Message}", ex);
            }
        }

        private void GetToRecipientsForSubmissionAsync(SqlConnection connection, SubmissionMailDataDto concernData, MailRecipientsDto recipients)
        {
            // Add only the reporting manager of assigned user
            if (!string.IsNullOrEmpty(concernData.Manager_Email))
            {
                recipients.ToRecipients.Add(new MailRecipientDto
                {
                    Email = concernData.Manager_Email.Trim(),
                    Name = concernData.Manager_Name
                });
            }
        }

        private async Task GetCcRecipientsForSubmissionAsync(SqlConnection connection, SubmissionMailDataDto concernData, MailRecipientsDto recipients)
        {
            ...
            // Get PU Head, Quality Manager, Manufacturing Manager, Quality Officer for the shop (same as assigned concern)
    ...
    // DTO for submission mail data
    public class SubmissionMailDataDto
    {
